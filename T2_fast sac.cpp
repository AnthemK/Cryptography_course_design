...
byte bin1k[1<<4], bin2k[1<<4];

void linear_attack(const byte xmask, const byte ymask, const byte bin1, const byte bin2, byte iniK) {
    memset(cnt, 0, sizeof(cnt));
    for (int i = 0; i < N; i++) {
        for (byte L1 = 0; L1 < 1<<4; L1++) bin1k[L1] = ((Sboxtransfer[0][ciphertext[i]^(L1<<bin1)]&ymask)>>bin1)&0xf;
        for (byte L2 = 0; L2 < 1<<4; L2++) bin2k[L2] = ((Sboxtransfer[0][ciphertext[i]^(L2<<bin2)]&ymask)>>bin2)&0xf;
        if (xmask == 0x0c00) for (byte L2 = 0; L2 < 1<<4; L2++) bin2k[L2] ^= ((Sboxtransfer[0][ciphertext[i]^iniK]&ymask)>>8)&0xf;
        for (byte L1 = 0; L1 < 1<<4; L1++)
            for (byte L2 = 0; L2 < 1<<4; L2++)
                cnt[L1][L2] += !(popcount[(plaintext[i]&xmask)^bin1k[L1]^bin2k[L2]]&1);
    }
...
int main() {
#ifndef ONLINE_JUDGE
    freopen(".\\TestCase\\T2\\5.in", "r", stdin);
