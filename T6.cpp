...
BigInt My_Fastest_Pow(BigInt Basee,BigInt Indexx,BigInt NowMod)       //指数压位实现快速幂 
{
	int qwer,cnt;
	cnt=0;
	for(cnt=0;Indexx!=0;++cnt)
	{
		...
		while(qwwq>0)
		{
			if((qwwq&qwer)>0) IndexBit[cnt]|=qwer,qwwq-=qwer;  //预处理压位之后的结果 
			qwer<<=1;
		}
		Indexx>>=6;
	}
...
BigInt My_Fast_Pow(BigInt Basee,BigInt Indexx,BigInt NowMod)       //滑动窗口优化快速幂 （但是好像有点奇怪（X）） 具体方法不再赘述 
{
	int cnt;
	for(cnt=0;Indexx!=0;++cnt)
	{
...
BigInt My_Pow_Prime(BigInt Basee,BigInt Indexx,BigInt NowMod)     //要求模数必须是质数，否则会出错 ，此处实现的是利用NAF表示来加速 
{
	...
	while(Indexx!=0)               //实际上因为我写的时候智力下线了，实际上这个实现是有问题的 。一种更优秀的方法应该是Basee和Inv保持不变，指数从高位到低位进行，中间Ans进行自乘取代Basee和Inv自乘 
	{
	...
	if(ifborr==1) Ans=(Ans*Basee)%NowMod;   //注意可能有溢出 
	return Ans;
}


BigInt CRT(BigInt ModResi[],BigInt ModNum[],int num)  //中国剩余定理 
{
